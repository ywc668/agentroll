...
def run_agent(question: str) -> str:
    """Run the agent: send question to Claude, execute tool calls, return diagnosis."""
    ...
    logger.info(f"Agent query: {question} | prompt={PROMPT_VERSION} model={MODEL_VERSION}")
    ...
    for i in range(max_iterations):
        ...
        if response.stop_reason == "tool_use":
            ...
            logger.info(f"Agent response complete | iterations={i+1}")
            return final_text
